  ...
  try {
    ...
    const items = (itemsRes.elements || [])
      ...
      .map(item => ({
        ...
        name: item.name,
        nameEs: null, // filled in after AI translation
        price: item.price || 0,
        emoji: "🍽️",
      }));
...
function GuestCheckTicket({ order, t, isQueue }) {
  ...
  const changeStyle = (changeType) => {
    switch (changeType) {
      case "added":     return { color: "#15803D", bg: "#F0FDF4", tagBg: null, label: null };
      case "increased": return { color: "#15803D", bg: "#F0FDF4", tagBg: null, label: null };
      case "decreased": return { color: "#D97706", bg: "#FFFBEB", tagBg: null, label: null };
...
function WaiterScreen({ menu, onOrderSent, lang }) {
  ...
  function addToCart(item) {
    setCart(prev => {
      ...
      // Store display name based on current language
      return [...prev, { ...item, displayName: getItemDisplayName(item), qty: 1 }];
    });
  }
  ...
  async function handleSend() {
    ...
    if (isEditing) {
      ...
      const order = {
        ...
        items: cart.map(i => ({ ...i, name: i.name })), // always store EN name in DB
...
export default function App() {
  ...
  const translationCache = useRef({}); // { itemId: translatedName }
  const t = T[lang];

  // Load menu from Clover on mount
  useEffect(() => {
    fetchMenuFromClover().then(async (rawMenu) => {
      setMenu(rawMenu);
      // If default lang is ES, translate immediately
      if (lang === "es") {
  ...
  // When lang changes, apply or revert translations
  useEffect(() => {
    if (!menu) return;
    if (lang === "es") {
      applyTranslations(menu);
    } else {
      // Revert to original English names
      setMenu(prev => ({
        ...prev,
  ...
  async function applyTranslations(currentMenu) {
    // Check which items need translation (not yet cached)
...
// ============================================================
// STYLES — Light Mode · #BE202E · Responsive
// ============================================================
const S = {
  // ROOT & NAV
  appRoot: {
  ...
  // TICKET (guest check) — kept cream/paper look intentionally for kitchen readability
  ticket: {
    background: "#FFFDF7",
  ...
  itemRow: { display: "flex", alignItems: "center", padding: "8px 16px", minHeight: 44 },
  itemQty: { width: 48, fontSize: 26, fontWeight: 900, letterSpacing: "-0.02em" },
  itemName: { flex: 1, fontSize: 20, lineHeight: 1.2 },
  changeTag: {
  ...
  menuItemActive: {
    ...
    boxShadow: "0 0 0 2px rgba(190,32,46,0.12)",
  },
