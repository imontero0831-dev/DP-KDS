...
try_deploy() {
  ...
  if $SSH "pi@$host" "test -f /home/pi/.failsafe-v2-installed" >/dev/null 2>&1; then
  ...
  if ! $SCP "$STAGING_LOCAL/tailscale-watchdog.sh" "$STAGING_LOCAL/install-failsafes.sh" "pi@$host:$STAGING_REMOTE/"; then
