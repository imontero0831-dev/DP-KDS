#!/bin/bash
# firstrun.sh for re-provisioning Drinks (kds-display-2)'s microSD from a fresh stock image,
...
#     --first-run-script pi-kiosk-failsafes/kds-drinks-firstrun.sh \
...
[connection]
id=preconfigured
uuid=B2C4E8A1-6D3F-4C9E-A1B7-3E5D9F2C8A4D
type=wifi
...
[connection]
id=fallback-hotspot
uuid=9D3A7B5C-2E4F-4A1D-8C6B-5F9E1A3D7C2B
type=wifi
...
# contending with NetworkManager for wlan0. Same conflict already confirmed and fixed on Kitchen's
# card the same week. Masking it here so a fresh image never has this fight in the first place.
...
# Wait for real internet, not just "network-online.target reached".
...
apt-get install -y xserver-xorg xinit openbox chromium chromium-common chromium-sandbox \
  rpi-chromium-mods unclutter xbindkeys xdotool
...
while true; do
  chromium \
    ...
    "https://dp-kds.vercel.app/?screen=drinks"
  sleep 2
done
CHROMEEOF
...
[Unit]
Description=DP-KDS Drinks kiosk stage-2 provisioning (runs once on first real boot)
