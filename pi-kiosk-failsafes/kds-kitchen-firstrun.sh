...
raspi-config nonint do_hostname kds-kitchen
...
# Belt-and-suspenders on the above: confirmed 2026-09-05 on Drinks' board that this exact drop-in,
# written here in stage 1 and verified byte-correct in firstrun.log with no error, was GONE from
# the filesystem after a real boot -- board came up to an interactive login prompt with no keyboard
# attached to answer it. Everything else stage 1 wrote in that same run (sudoers, both
...
# --no-install-recommends was used) running a postinst that resets tty1's getty config. Rather than
# chase the exact package, make autologin unconditionally self-healing instead: a tiny oneshot that
...
# for the card's entire run. The original boards all have this (see
# reference_dpkds_kiosk_pi_fleet.md); it was simply never carried into this re-provisioning
# script. Every watchdog script's sudo calls are silently useless without this.
...
# on their own, but wifi-signal.log on Drinks' card showed wlan0 never appearing in
# /proc/net/wireless at all after 2026-09-05's fresh re-flash (not a slow-association issue --
# the interface itself never came up), and this is a second, independent mechanism worth having
# in place at the same time in case the keyfile path alone isn't what actually brings the radio
# up on this hardware.
...
# months before this script existed). `tailscale up` used to need a manual interactive
...
tailscale up --authkey=tskey-auth-kzoBvVwJBo11CNTRL-ZFYhnueLnmJoJA1Je43omJvHctfNSNHJ --hostname=kds-kitchen || true
